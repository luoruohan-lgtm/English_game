                  ...
                  <div className="space-y-4">
                    {storyNarratives.slice(0, novelStep + 1).map((seg, idx) => {
                      ...
                      return (
                        ...
                        >
                          <p className="indent-6 leading-relaxed mb-3">{seg.intro}</p>
                          ...
                          {isCurrentBlock && (
                            <div className="mt-4 pt-4 border-t border-slate-200/60 text-left space-y-3">
                              <div className="p-3 bg-white rounded-xl border border-slate-200 flex flex-col md:flex-row md:items-start justify-between gap-3 shadow-2xs">
                                <div className="space-y-1.5 flex-1">
                                  <span className="text-[11px] font-bold text-indigo-650 block">🤔 情境句式选择解锁：</span>
                                  <div className="text-xs text-slate-700">
                                    <span className="font-bold text-slate-500 mr-1">句中核心词:</span>
                                    <span className="font-mono bg-indigo-50 font-black text-indigo-700 px-1.5 py-0.5 rounded border border-indigo-100">{seg.wordInContext}</span>
                                  </div>

                                  {/* Render dynamic contextual sentence paired from sentences.ts */}
                                  {(() => {
                                    const match = WORD_SENTENCES[seg.wordInContext.toLowerCase()];
                                    if (!match) return null;
                                    return (
                                      <div className="mt-1.5 p-3 bg-slate-50 border border-slate-150 rounded-xl space-y-1.5 text-left">
                                        <div className="text-[10px] font-bold text-indigo-750">📚 真题语境造句:</div>
                                        <p 
                                          className="text-xs text-slate-900 font-medium leading-relaxed"
                                          dangerouslySetInnerHTML={{ __html: highlightWordInSentence(match.sentence, seg.wordInContext) }}
                                        />
                                        <p className="text-[11px] text-slate-500 font-medium leading-relaxed italic">
                                          意译：{match.translation}
                                        </p>
                                      </div>
                                    );
                                  })()}
                                ...
                                <button
                                  ...
                                  className={`px-3 py-1.5 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-1 cursor-pointer ${
                                    isPlayingAudio === seg.wordInContext ? "bg-amber-100 text-amber-800 border-amber-300" : "bg-slate-50 border-slate-250 text-slate-500"
                                  }`}
                                >
                                  <Volume2 className="w-3.5 h-3.5" />
                                  <span>{isPlayingAudio === seg.wordInContext ? "音频演绎中..." : "美声发读"}</span>
                                </button>
                              </div>
